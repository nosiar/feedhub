...
import type { TelegramChat } from "../api.js";
import { fetchTelegramChats } from "../api.js";
...
export function TelegramChatManager({
  ...
  chats: TelegramChat[];
  onChange: (chats: TelegramChat[]) => void;
}) {
  const [allChats, setAllChats] = useState<
    { id: string; name: string; type: string; unreadCount: number }[] | null
  >(null);
  ...
  const filtered = (allChats ?? []).filter(
    (c) => !selectedIds.has(c.id) && c.name.toLowerCase().includes(search.toLowerCase())
  );
  ...
  return (
    <div style={{ padding: 16, borderBottom: "1px solid #f0f0f0" }}>
      ...
      >
        <h3 style={{ fontSize: 15, margin: 0 }}>✈️ Telegram</h3>
        <span
          onClick={handleLoadChats}
          style={{ color: "#4285F4", fontSize: 12, cursor: "pointer" }}
        >
          + 채널 추가
        </span>
      </div>
